ci.reliability.bs<- function(data, type="Congeneric", analysis.type="Factor Analytic", conf.level=.95, B=100, ...)
{
...
.bs.reliability.ci <- function(data, g, type=type, analysis.type=analysis.type, conf.level=conf.level)
        {
        ci.results<-ci.reliability(data=data[g,], type=type, analysis.type=analysis.type, conf.level=conf.level)
        return(ci.results$Estimated.reliability)
        }

    # Output of bootstrap
boot.out <- boot(data=data, statistic=.bs.reliability.ci, R=B, stype="i", type=type, analysis.type=analysis.type, conf.level=conf.level)
