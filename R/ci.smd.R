...
if(length(ncp)==1)
{
#if(ncp==0) stop("You needed use a noncentral method since the noncentrality parameter is zero; use the critical value from the central t-distribution.", call.=FALSE)
...
if(length(smd)==1)
{
#if(smd==0) stop("You needed use a noncentral method since the effect size is zero; use the critical value from the central t-distribution.", call.=FALSE)
