ci.reliability <- function (S = NULL, data = NULL, N = NULL, model = "True-Score Equivalent", 
    type = "Factor Analytic", conf.level = 0.95, interval = TRUE, 
    Bootstrap = FALSE, B = 10000, BootstrapCI = "BCa") 
{
    if (!require(sem)) 
    ...
    if (Bootstrap == FALSE) {
        if (type == "Factor Analytic") {
            ...
            if (sum(model == model.type1) == 1) 
                result <- CFA.1(S = S, N = as.numeric(N), equal.loading = TRUE, 
                  equal.error = TRUE)
            if (sum(model == model.type2) == 1) 
                result <- CFA.1(S = S, N = as.numeric(N), equal.loading = TRUE, 
                  equal.error = FALSE)
            if (sum(model == model.type3) == 1) 
                result <- CFA.1(S = S, N = as.numeric(N), equal.loading = FALSE, 
                  equal.error = FALSE)
            ...
            if (p == 1) {
                ...
                var.v <- sum(result$Parameter.cov[(k - q + 1):k, 
                  (k - q + 1):k])
            }
            if (l != 1 & p != 1) 
                cov.u.v <- (sum(result$Parameter.cov) - var.v - 
                  var.u)/2
            if (l == 1 & p != 1) 
                cov.u.v <- sum(result$Parameter.cov[2:k, 1]) * 
                  q
            if (l == 1 & p == 1) 
            ...
            if (interval) {
                D1 <- 2 * u * v/(u^2 + v)^2
                D2 <- (-1) * u^2/(u^2 + v)^2
                se.omega <- sqrt(D1^2 * var.u + D2^2 * var.v + 
                  2 * D1 * D2 * cov.u.v)
        ...
        if (type == "Normal Theory") {
            ...
            if (!is.null(data)) {
                ...
                if (dim(data)[1] == dim(data)[2]) {
                  if (sum(round(data, 4) == t(round(data, 4))) == 
            ...
            if (sum(model == model.type1) == 1) {
                ...
                if (interval) {
                  ...
                  variance <- (2 * (1 - alpha)^2 * p)/((N - 1) * 
                    (p - 1))
                  SE <- as.vector(sqrt((2 * (1 - alpha)^2 * p)/((N - 1) * 
                    (p - 1))))
            ...
            if (sum(model == model.type2) == 1) {
                ...
                if (interval) {
                  ...
                  gamma.2.1.2 <- ((sum(diag(cor.mat %*% cor.mat))) + 
                    (sum(diag(cor.mat)))^2)
                  gamma.2.1 <- gamma.2.1.1 * gamma.2.1.2
                  gamma.2.2 <- 2 * (sum(diag(cor.mat))) * (t(j) %*% 
                    (cor.mat %*% cor.mat) %*% j)
        ...
        BootstrapCI.type1 <- c("perc", "Perc", "percentile", 
            "Percentile", "percentile ci", "Percentile ci", "Percentile CI", 
            "percentile CI")
        BootstrapCI.type2 <- c("bca", "Bca", "BCa", "BCA", "Bias-corrected and acceleration", 
            "Bias-Corrected and Acceleration", "bias-corrected and acceleration")
        if (sum(BootstrapCI == BootstrapCI.type1, BootstrapCI == 
            BootstrapCI.type2) != 1) 
        ...
        if (type == "Factor Analytic") {
            ...
            .bs.omega <- function(data, model, i) {
                ...
                if (p == 1) {
                  ...
                  v <- sum(result$Indicator.var)
                  var.v <- sum(result$Parameter.cov[(k - q + 
                    1):k, (k - q + 1):k])
                }
                if (l != 1 & p != 1) 
                  cov.u.v <- (sum(result$Parameter.cov) - var.v - 
                ...
                if (l == 1 & p != 1) 
                  cov.u.v <- sum(result$Parameter.cov[2:k, 1]) * 
                    q
                if (l == 1 & p == 1) 
                  cov.u.v <- result$Parameter.cov[2, 1] * q * 
                    q
                omega <- u^2/(u^2 + v)
                return(omega)
            }
            boot.out <- boot(data = data, statistic = .bs.omega, 
                R = B, stype = "i", model = model)
            se.omega <- sd(boot.out$t)
            CI.output <- NULL
            CI.lower <- NULL
            CI.upper <- NULL
            if (sum(BootstrapCI == BootstrapCI.type1) == 1) {
                CI.output <- boot.ci(boot.out = boot.out, conf = conf.level, 
                  type = "perc")$perc
    	    ...
    	    BCA.Output <- boot.ci(boot.out = boot.out, conf = conf.level, 
        	type = "bca")[4:5]
    	    Percentile.Output <- boot.ci(boot.out = boot.out, conf = conf.level, 
        	type = "perc")[4:5]
    	    ...
    	    colnames(Output) <- c("Desired.Conf.Level", "Lower.Limit.Index", 
        	"Upper.Limit.Index", "Lower.Conf.Limit", "Upper.Conf.Limit")
            ci <- list(CI.lower = CI.lower, CI.upper = CI.upper, 
                Estimated.reliability = omega, SE.reliability = se.omega, 
                Conf.Level = conf.level, Summary = Output)
            return(ci)
        }
        if (type == "Normal Theory") {
            if (sum(model == model.type3) == 1) 
            ...
            boot.out <- boot(data = data, statistic = .bs.alpha, 
                R = B, stype = "i")
